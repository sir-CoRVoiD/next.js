...
- `res.status(code).redirect(path)` - Redirects to a specified path or URL. `status code` must be a valid [HTTP status code](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes). If not specified, `status code` defaults to "307" "Temporary redirect".
